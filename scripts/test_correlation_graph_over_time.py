"""
Test correlation-based graph over multiple rolling windows.

Run from project root:
    python -m scripts.test_correlation_graph_over_time

This script:
- loads S&P 500 data
- builds the universe & rectangular return panel
- iterates over multiple rolling windows
- for each window, constructs the correlation graph
...
def compute_graph_metrics(G: graphs_correlation.GraphData) -> dict:
    """
...
def main():
    ...
    for idx, rw in enumerate(rw_gen.iter_windows()):
        # Implement stride (e.g., use every 5th window)
    ...
    # 5. Print summary
    # ------------------------------------------------------------------
    print("\n=== Summary over processed windows ===")
    ...
    # Optionally, save to CSV for plotting later
    out_path = "graph_metrics_over_time.csv"
